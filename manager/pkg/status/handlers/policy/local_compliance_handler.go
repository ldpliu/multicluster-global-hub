...
func RegisterLocalPolicyComplianceHandler(c client.Client, conflationManager *conflator.ConflationManager) {
...
func postCompliancesData(
	...
	if configs.IsInventoryAPIEnabled() {
		...
		mchVersion, err := utils.GetClusterClaimValueFromCluster(
			ctx,
			c,
			leafHubName,
		...
		if err != nil {
			log.Warnf("failed to get version from cluster claim: %v", err)
		}
