...
func (h *localPolicySpecHandler) postPolicyToInventoryApi(
	...
	if len(createPolicy) > 0 {
		createPolicyToInventory = make([]policiesv1.Policy, len(createPolicy))
	...
	if len(updatePolicy) > 0 {
		updatePolicyToInventory = make([]policiesv1.Policy, len(updatePolicy))
	...
	if len(deletePolicy) > 0 {
		deletePolicyInInventory = make([]models.ResourceVersion, len(deletePolicy))
		for _, policy := range deletePolicy {
...
func getPolicyIdToVersionMap(db *gorm.DB, leafHubName string) (map[string]models.ResourceVersion, error) {
	...
	err := db.Select(
		"payload->'metadata'->>'uid' AS key, concat(payload->'metadata'->>'namespace', '/', payload->'metadata'->>'name') AS name, payload->'metadata'->>'resourceVersion' AS resource_version").
