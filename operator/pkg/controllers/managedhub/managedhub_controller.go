...
func StartController(initOption config.InitOption) (bool, error) {
	err := NewManagedHubController(initOption.Mgr).SetupWithManager(initOption.Mgr)
	if err != nil {
		return false, err
	}
...
func (r *ManagedHubController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if reconcileErr = utils.AnnotateManagedHubCluster(ctx, r.GetClient()); reconcileErr != nil {
...
func NewManagedHubController(mgr ctrl.Manager) *ManagedHubController {
	return &ManagedHubController{Manager: mgr}
}
