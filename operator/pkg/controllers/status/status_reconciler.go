...
func (r *StatusReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	err = r.updateDeploymentComponents(ctx, mgh.Namespace, componentsStatus)
	if err != nil {
		klog.Errorf("Failed to update components status:%v", err)
		return ctrl.Result{}, err
	}

	// update postgres
	err = r.updateStatefulsetComponents(ctx, mgh.Namespace, componentsStatus)
	if err != nil {
		klog.Errorf("Failed to update components status:%v", err)
		return ctrl.Result{}, err
	}
	...
	if !config.IsBYOKafka() {
		needRequeue, err := r.updateKafkaComponents(ctx, mgh.Namespace, componentsStatus)
		if err != nil {
			klog.Errorf("Failed to update components status:%v", err)
...
func (r *StatusReconciler) updateMghStatus(ctx context.Context, componentsStatus map[string]v1alpha4.StatusCondition) error {
	return retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		// update components
		updatedComponents, desiredComponentsStatus := r.needUpdateComponentsStatus(curmgh.Status.Components, componentsStatus)
		...
		return r.Client.Status().Update(ctx, curmgh)
	})
}

// needUpdatePhase check if the phase need updated. phase is running only when all the components available
func needUpdatePhase(mgh *v1alpha4.MulticlusterGlobalHub, desiredComponentsStatus map[string]v1alpha4.StatusCondition) (bool, v1alpha4.GlobalHubPhaseType) {
...
func (r *StatusReconciler) needUpdateComponentsStatus(currentComponentsStatus, desiredComponentsStatus map[string]v1alpha4.StatusCondition) (bool, map[string]v1alpha4.StatusCondition) {
...
func (r *StatusReconciler) updateKafkaComponents(ctx context.Context, namespace string, componentsStatus map[string]v1alpha4.StatusCondition) (bool, error) {
...
func (r *StatusReconciler) updateStatefulsetComponents(ctx context.Context, namespace string, componentsStatus map[string]v1alpha4.StatusCondition) error {
	statefulsetList := &appsv1.StatefulSetList{}
	err := r.Client.List(ctx, statefulsetList, &client.ListOptions{
		Namespace: namespace,
	})
	if err != nil {
	...
	for _, deploy := range statefulsetList.Items {
		if deploy.Name != config.COMPONENTS_POSTGRES_NAME {
			continue
		}

		if deploy.Status.AvailableReplicas == *deploy.Spec.Replicas {
			componentsStatus[deploy.Name] = v1alpha4.StatusCondition{
				Kind:               "StatefulSet",
				Name:               deploy.Name,
				Type:               config.COMPONENTS_AVAILABLE,
				Status:             config.CONDITION_STATUS_TRUE,
				Reason:             config.MINIMUM_REPLICAS_AVAILABLE,
				Message:            fmt.Sprintf("Component %s have been deployed", deploy.Name),
				LastTransitionTime: metav1.Time{Time: time.Now()},
			}
			continue
		}
		componentsStatus[deploy.Name] = v1alpha4.StatusCondition{
			...
			Message:            fmt.Sprintf("Component %s have been deployed, but not ready", deploy.Name),
			LastTransitionTime: metav1.Time{Time: time.Now()},
		}
	}
	return nil
...
func (r *StatusReconciler) updateDeploymentComponents(ctx context.Context, namespace string, componentsStatus map[string]v1alpha4.StatusCondition) error {
	deploymentList := &appsv1.DeploymentList{}
	err := r.Client.List(ctx, deploymentList, &client.ListOptions{
		Namespace: namespace,
	})
	if err != nil {
		klog.Errorf("Failed to list deployments")
