...
func (r *KafkaController) isTransportReady(ctx context.Context, c client.Client, namespace, name string) (config.ComponentStatus, error) {
	if r.trans.isKafkaReady {
		return config.ComponentStatus{
			Ready:  true,
			Kind:   "Kafka",
			Reason: config.CONDITION_REASON_KAFKA_READY,
			Msg:    config.CONDITION_MESSAGE_KAFKA_READY,
		}, nil
	}
	reason := "KafkaNotReady"
	message := "Kafka cluster is not ready"
	if r.trans.kafkaReason != "" {
		reason = r.trans.kafkaReason
	}
	if r.trans.kafkaMsg != "" {
		message = r.trans.kafkaMsg
	}
	return config.ComponentStatus{
		Ready:  false,
		Kind:   "Kafka",
