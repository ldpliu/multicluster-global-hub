...
type strimziTransporter struct {
	...
	// used to update mgh kafka component status
	isKafkaReady bool
	kafkaReason  string
	kafkaMsg     string
}
...
func (k *strimziTransporter) isCSVInstalled(ns string) (bool, error) {
...
func (k *strimziTransporter) EnsureKafka() (bool, error) {
	...
	installed, err := k.isCSVInstalled(k.mgh.Namespace)
	if err != nil {
		return true, err
	}
	if !installed {
...
func (k *strimziTransporter) kafkaClusterReady() (bool, error) {
	...
	for _, condition := range kafkaCluster.Status.Conditions {
		if *condition.Type == "Ready" {
			...
			k.kafkaMsg = *condition.Message
			k.kafkaReason = *condition.Reason
			return false, nil
		}
	}
