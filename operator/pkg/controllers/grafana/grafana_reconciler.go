...
func StartController(initOption config.InitOption) (bool, error) {
	...
	err := NewGrafanaReconciler(initOption.Mgr, initOption.KubeClient).SetupWithManager(initOption.Mgr)
	if err != nil {
		return false, err
	}
...
func (r *GrafanaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	defer func() {
		config.UpdateMghComponentStatus(ctx, reconcileErr, r.client,
			mgh, config.COMPONENTS_GRAFANA_NAME,
