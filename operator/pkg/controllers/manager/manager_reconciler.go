...
func StartController(initOption config.InitOption) (bool, error) {
	...
	err := NewManagerReconciler(initOption.Mgr, initOption.KubeClient, initOption.OperatorConfig).SetupWithManager(initOption.Mgr)
	if err != nil {
		return false, err
	}
...
func (r *ManagerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).Named("managerController").
...
func (r *ManagerReconciler) Reconcile(ctx context.Context,
	...
	defer func() {
		config.UpdateMghComponentStatus(ctx, reconcileErr, r.GetClient(),
			mgh, config.COMPONENTS_MANAGER_NAME,
	...
	res, reconcileErr = r.setUpMetrics(ctx, mgh)
